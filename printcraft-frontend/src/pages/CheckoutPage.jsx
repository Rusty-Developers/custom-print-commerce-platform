...
export default function CheckoutPage() {
  ...
  const handleAddressSaved = (addr) => { setAddresses(p => [...p, addr]); setSelectedAddressId(addr.id); setShowAddForm(false) }
      ...
      <div className="container">
        <div className="checkout-layout">
          ...
          <div>
              ...
              <>
                {addresses.map((addr) => (
                  <div key={addr.id} className={`address-card${selectedAddressId===addr.id?' selected':''}`}
                    onClick={() => setSelectedAddressId(addr.id)} id={`address-${addr.id}`}>
                    <div style={{ display:'flex', alignItems:'flex-start', gap:10 }}>
                      <div style={{ width:18, height:18, borderRadius:'50%', border:`2px solid ${selectedAddressId===addr.id?'var(--primary)':'var(--border-color)'}`, background:selectedAddressId===addr.id?'var(--primary)':'white', flexShrink:0, marginTop:2 }} />
                      <div>
                ...
                {!showAddForm ? (
                  <button className="add-address-btn" onClick={() => setShowAddForm(true)} id="add-address-btn">+ Add New Address</button>
                ) : (
            ...
            {/* Trust Badge */}
            <div style={{ display:'flex', alignItems:'center', gap:10, padding:'12px 16px', background:'#F0FDF4', border:'1px solid #BBF7D0', borderRadius:8, marginTop:20 }}>
              <span style={{ fontSize:18 }}>🔒</span>
              <img src="https://razorpay.com/assets/razorpay-glyph.svg" alt="Razorpay" style={{ height:22, width:'auto' }} onError={e=>e.target.style.display='none'} />
              <span style={{ fontSize:13, fontWeight:600, color:'#166534' }}>100% Secure Payment — Payments secured by Razorpay</span>
            </div>

            <div className="notice-box" style={{ marginTop:16 }}>
              ⚠️ <strong>Important:</strong> Orders cannot be cancelled once confirmed. No returns after delivery. Prepaid only — no COD.
            </div>
          </div>
            ...
            <div className="order-summary-card">
              ...
              <div className="order-summary-body">
                {cart.map((item, i) => (
                  <div key={i} style={{ display:'flex', gap:12, paddingBottom:12, borderBottom:'1px solid var(--divider)' }}>
                    <img src={item.uploadedImageDataUrl || item.imageUrl || 'https://placehold.co/60x60'} alt={item.productName}
              ...
              <div style={{ padding:'0 20px 20px' }}>
                <button className="btn btn-primary btn-full btn-lg" onClick={handlePlaceOrder}
                  disabled={placing || !selectedAddressId} id="place-order-btn">
